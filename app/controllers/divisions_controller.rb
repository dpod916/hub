class DivisionsController < ApplicationController
  ...
  # GET /divisions.json
  def index
    items = Division.includes(:profile)
    respond_to do |format|
      format.html { render "layouts/index", locals: {items: items, items_name: 'divisions'}  }
  ...
  def show
     @activities = @division.group_activities
                      .paginate(:page => params[:page], :per_page => 10000)
                      .group_by {|t| t.created_at.to_date }
    respond_to do |format|
      format.html { render "layouts/show", locals: {item: @division}  }
      format.json
    end
  end
