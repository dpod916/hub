		...
		if line > 1
			s.first_column.upto(s.last_column) do |column|
				unless s.cell(1, column).nil?
				  stringer = s.cell(line, column)
					if ! striner.valid_encoding?
					  stringer = stringer.encode("UTF-16be", :invalid=>:replace, :replace=>"?").encode('UTF-8')
					end
				  attribute_params[s.cell(1, column).downcase.to_sym] = stringer
				end
			end
